...
func StatPage(strJSON []byte) (bytes.Buffer, error) {
	//defer func() {
	...
	if err != nil {
		...
		if err != nil {
			return b, fmt.Errorf("func StatPage: a error occurred rendering template statPage.html: %s", err)
		}
	...
	err = ts.ExecuteTemplate(&b, "statPage.html", p)
	if err != nil {
		return b, fmt.Errorf("an error occured execute template statPage.html: %s", err)
	}
