...
func RedirectPage(shortLink *Redirect) (*Redirect, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("func StatPage: error occured json marshaling stat page: %s", err)
	}
	...
	if err = json.NewDecoder(res.Body).Decode(&respRedirect); err != nil {
		return nil, fmt.Errorf("error unmarshal request: %s, status code: %d", err, http.StatusInternalServerError)
	}

	return respRedirect, nil
}
